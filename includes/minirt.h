...
int			parse_material(t_minirt *rt, t_material *material,
				char *material_tokens[]);
...
t_color		calculate_reflection(t_ray ray, t_hit *hit,
				t_scene *scene, int depth);
t_color		calculate_refraction(t_ray ray, t_hit *hit,
				t_scene *scene, int depth);
...
t_vec3		apply_bump_map_if_present(t_hit *hit,
				t_vec3 normal, double u, double v);
...
void		handle_parse_error(char *filename, int line_num,
				char *message, t_minirt *rt);
...
void		get_cube_bounds(t_object *cube,
				t_vec3 *cube_min, t_vec3 *cube_max);
